...
┌─────────────────────────────────────────────────────────────────────┐
│  权重量化 (Weight Quantization) - 可变位宽 W2/W4/W8                 │
├─────────────────────────────────────────────────────────────────────┤
│  • W2 (2-bit): 低敏感层，使用 2-bit 对称量化，最大压缩              │
│  • W4 (4-bit): 中敏感层，使用 4-bit 对称量化，平衡压缩              │
│  • W8 (8-bit): 高敏感层，使用 8-bit 对称量化，保持精度              │
│  • 分组量化 (group_size=128)，每组独立计算 scale                    │
│  • 对称量化模式，权重分布通常对称                                   │
└─────────────────────────────────────────────────────────────────────┘
...
│  • 非对称量化模式 (per-tensor)，激活值分布通常不对称                │
│  • 固定 A8 可简化硬件实现，同时保证推理精度                         │
└─────────────────────────────────────────────────────────────────────┘

⚠️ 重要说明：模拟量化 vs 真实量化
---------------------------------

本模块实现的是【模拟量化】（Fake Quantization），而非【真实量化】。

┌─────────────────────────────────────────────────────────────────────┐
│  模拟量化 (Simulated Quantization) - 本模块实现                     │
├─────────────────────────────────────────────────────────────────────┤
│  FP32 权重 → 量化(round) → 反量化 → FP32 权重                       │
│                                                                     │
│  数据类型始终是 FP32，只是模拟了量化带来的精度损失                   │
│  ❌ 不会加速推理（反而因为额外操作而变慢）                           │
│  ✅ 用于评估量化对模型精度的影响                                     │
│  ✅ 用于搜索最优的逐层位宽配置                                       │
└─────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────┐
│  真实量化 (Real Quantization) - 需要使用专门的推理框架               │
├─────────────────────────────────────────────────────────────────────┤
│  FP32 权重 → 转换为 INT4/INT8 → 存储为 GGUF/GPTQ 格式               │
│                                                                     │
│  计算直接使用低精度整数运算（硬件加速）                              │
│  ✅ 真正加速推理（5-10 倍）                                          │
│  ✅ 大幅减少内存占用（70-85%）                                       │
│  推荐框架: llama.cpp, bitsandbytes, TensorRT-LLM                    │
└─────────────────────────────────────────────────────────────────────┘

本模块功能：
-----------
- 模拟量化函数：将张量量化后再反量化，模拟精度损失
- 分组量化：按 group_size 分组，每组独立计算 scale
- 对称/非对称量化模式

核心概念：
---------
1. 对称量化 (Symmetric): 量化范围 [-qmax, qmax]，无零点偏移
   - 适用于权重量化 (W2/W4/W8)，分布通常对称
   - scale = max(|x|) / (2^(n-1) - 1)
...
class MixedPrecisionLinear(nn.Module):
    ...
    量化策略：W2/W4/W8 + A8
    ----------------------
    - 权重 (Weight): 可选 W2 (2-bit), W4 (4-bit), W8 (8-bit)
    - 激活 (Activation): 固定使用 A8 (8-bit)
    - 权重使用对称量化 + 分组量化 (group_size=128)
    - 激活使用非对称量化 (per-tensor)
