...
本优化器搜索最优的权重位宽配置 (W2/W4/W8)，激活值固定使用 A8：

  • W2 (2-bit 权重) + A8: 适用于低敏感层，最大压缩
  • W4 (4-bit 权重) + A8: 适用于中敏感层，平衡压缩  
  • W8 (8-bit 权重) + A8: 适用于高敏感层，保持精度
...
- 目标1：最大化模型压缩率
- 目标2：最小化量化精度损失
- 约束条件：激活值固定使用 A8 (8-bit)

遗传算法流程：
-------------
1. 初始化种群：随机生成多个位宽配置
2. 适应度评估：基于层敏感度计算总MSE (激活始终为A8)
3. 选择：保留适应度高的个体
4. 交叉：两个父代生成子代
5. 变异：随机改变部分基因
6. 迭代：重复2-5直到收敛
...
class MixedPrecisionGA:
    """
    混合精度遗传算法优化器 - 实现 W2/W4/W8 + A8 策略优化
    
    用于找到最优的逐层权重量化位宽配置 (W2/W4/W8)，激活固定使用 A8。
    在模型压缩率和精度之间寻找平衡。
    
    量化策略：W2/W4/W8 + A8
    ----------------------
    - 权重 (Weight): 优化搜索 W2, W4, W8 最优配置
    ...
    def __init__(self, n_layers: int, population_size: int = 20, 
        ...
        # 可选权重位宽: W2(激进压缩)+A8, W4(平衡)+A8, W8(近似FP16)+A8
...
class LayerSensitivityAnalyzer:
    """
    层敏感度分析器 - 为 W2/W4/W8 + A8 策略提供敏感度评估
    
    评估每层对不同权重量化位宽 (W2/W4/W8) 的敏感程度，激活固定使用 A8。
    用于指导混合精度配置的选择。
    
    量化策略：W2/W4/W8 + A8
    ----------------------
    - 权重 (Weight): 测试 W2, W4, W8 不同位宽
    - 激活 (Activation): 固定使用 A8 (8-bit)
    
    敏感度定义：
    -----------
    敏感度 = MSE(量化输出, 原始输出)
    其中量化使用 Wx + A8 配置（x = 2, 4, 8）
    
    敏感度越高的层应使用更高的位宽，敏感度低的层可使用W2激进压缩。
    
    敏感度分类：
    -----------
    - 低敏感度 (MSE < 0.1): 可安全使用 W2 + A8
    - 中敏感度 (0.1 <= MSE < 0.5): 建议使用 W4 + A8
    - 高敏感度 (MSE >= 0.5): 应保持 W8 + A8
    """
